...
namespace ts.GoToDefinition {
    /// Goto definition
    //name
    //todo: try to reduce inputs
    export function getYourselfADangOlDefinitionAtPosition(program: Program, sourceFile: SourceFile, position: number): DefinitionInfo[] {
        ...
        if (node.parent.kind === SyntaxKind.ShorthandPropertyAssignment) {
            ...
            const shorthandSymbolKind = getSymbolKind(typeChecker, shorthandSymbol, node);
    ...
    //todo: check that these have the same order as they did before
    ...
    function getSymbolInfo(typeChecker: TypeChecker, symbol: Symbol, node: Node) {
        return {
            ...
            symbolKind: getSymbolKind(typeChecker, symbol, node),
