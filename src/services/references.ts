/* @internal */
namespace ts.References {
    //name
    //todo: don't take program as input
    export function searchForReferences(program: Program, cancellationToken: CancellationToken, sourceFile: SourceFile, position: number, findInStrings: boolean, findInComments: boolean): ReferencedSymbol[] | undefined {
        ...
        switch (node.kind) {
            ...
            case SyntaxKind.StringLiteral:
                return getReferencedSymbolsForNode(program.getTypeChecker(), cancellationToken, node, program.getSourceFiles(), findInStrings, findInComments);
        }
        return undefined;
    }
    ...
    export function getReferencedSymbolsForNode(typeChecker: TypeChecker, cancellationToken: CancellationToken, node: Node, sourceFiles: SourceFile[], findInStrings: boolean, findInComments: boolean): ReferencedSymbol[] {
        ...
        if (!symbol && node.kind === SyntaxKind.StringLiteral) {
            return getReferencesForStringLiteral(typeChecker, <StringLiteral>node, sourceFiles);
        }
        ...
        function getDefinition(symbol: Symbol): ReferencedSymbolDefinitionInfo {
            const info = getSymbolDisplayPartsDocumentationAndSymbolKind(typeChecker, symbol, node.getSourceFile(), getContainerNode(node), node);
        ...
        function getReferencesForStringLiteral(typeChecker: TypeChecker, node: StringLiteral, sourceFiles: SourceFile[]): ReferencedSymbol[] {
