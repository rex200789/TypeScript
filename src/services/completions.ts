...
namespace ts.Completions {
    ...
    function getCompletionData(typeChecker: TypeChecker, log: Log, sourceFile: SourceFile, position: number) {
    ...
    export function getCompletionsAtPosition(typeChecker: TypeChecker, log: Log, compilerOptions: CompilerOptions, sourceFile: SourceFile, position: number): CompletionInfo {
        ...
        if (isSourceFileJavaScript(sourceFile)) {
            const uniqueNames = getCompletionEntriesFromSymbols(typeChecker, compilerOptions, symbols, entries, location, /*performCharacterChecks*/ false);
            ...
            getCompletionEntriesFromSymbols(typeChecker, compilerOptions, symbols, entries, location, /*performCharacterChecks*/ true);
        }
        ...
        function createCompletionEntry(typeChecker: TypeChecker, compilerOptions: CompilerOptions, symbol: Symbol, location: Node, performCharacterChecks: boolean): CompletionEntry {
        ...
        function getCompletionEntriesFromSymbols(typeChecker: TypeChecker, compilerOptions: CompilerOptions, symbols: Symbol[], entries: CompletionEntry[], location: Node, performCharacterChecks: boolean): Map<string> {
            const start = timestamp();
            const uniqueNames = createMap<string>();
            if (symbols) {
                for (const symbol of symbols) {
                    const entry = createCompletionEntry(typeChecker, compilerOptions, symbol, location, performCharacterChecks);
                    if (entry) {
        ...
        function getStringLiteralCompletionEntriesFromPropertyAssignment(element: ObjectLiteralElement) {
            ...
            const entries: CompletionEntry[] = [];
            if (type) {
                getCompletionEntriesFromSymbols(typeChecker, compilerOptions, type.getApparentProperties(), entries, element, /*performCharacterChecks*/false);
                if (entries.length) {
        ...
        function getStringLiteralCompletionEntriesFromElementAccess(node: ElementAccessExpression) {
            ...
            const entries: CompletionEntry[] = [];
            if (type) {
                getCompletionEntriesFromSymbols(typeChecker, compilerOptions, type.getApparentProperties(), entries, node, /*performCharacterChecks*/false);
                if (entries.length) {
                    return { isMemberCompletion: true, isNewIdentifierLocation: true, entries };
                }
            }
    ...
    export function getCompletionEntryDetails(typeChecker: TypeChecker, log: Log, compilerOptions: CompilerOptions, sourceFile: SourceFile, position: number, entryName: string): CompletionEntryDetails {
        ...
        if (completionData) {
            ...
            // completion entry.
            const symbol = forEach(symbols, s => getCompletionEntryDisplayNameForSymbol(typeChecker, s, target, /*performCharacterChecks*/ false, location) === entryName ? s : undefined);
