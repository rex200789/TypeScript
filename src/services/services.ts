    ...
    export function createLanguageService(host: LanguageServiceHost,
        ...
        function getCompletionsAtPosition(fileName: string, position: number): CompletionInfo {
            ...
            function createCompletionEntry(symbol: Symbol, location: Node, performCharacterChecks: boolean): CompletionEntry {
                ...
                return {
                    name: displayName,
                    kind: SymbolDisplay.getSymbolKind(program.getTypeChecker(), symbol, location),
        ...
        function getCompletionEntryDetails(fileName: string, position: number, entryName: string): CompletionEntryDetails {
            ...
            if (completionData) {
                ...
                if (symbol) {
                    const { displayParts, documentation, symbolKind } = SymbolDisplay.getSymbolDisplayPartsDocumentationAndSymbolKind(typeChecker, symbol, getValidSourceFile(fileName), location, location, SemanticMeaning.All);
                    return {
                        name: entryName,
                        kindModifiers: SymbolDisplay.getSymbolModifiers(symbol),
                        kind: symbolKind,
        ...
        function getDocumentHighlights(fileName: string, position: number, filesToSearch: string[]): DocumentHighlights[] {
            ...
            const sourceFile = getValidSourceFile(fileName);
            return DocumentHighlights.getMeSomeDocumentHighlights(program.getTypeChecker(), cancellationToken, sourceFile, position, sourceFilesToSearch);
        }
